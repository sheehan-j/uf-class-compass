					...
					{searchResult.sections.map((section) => (
						...
						>
							...
							<div className="flex flex-row gap-2 justify-between items-center">
									...
									<div className="text-end flex flex-row items-center"> <span className="font-semibold mr-1">Rating: </span>{JSON.stringify(section.instructor.rmpData.rating)} <span className="ml-2 ">{renderStars(section.instructor.rmpData.rating)}</span></div>
									<div className="text-end"> <span className="font-semibold">Difficulty: </span>{JSON.stringify(section.instructor.rmpData.difficulty)} </div>
